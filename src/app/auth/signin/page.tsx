...
import { signInWithGitHub } from "@/app/auth/signin/github-action";
import { signInErrorMessage } from "@/lib/auth-public-error";
import { normalizeNip07PubkeyHex } from "@/lib/nip07-pubkey";

function SignInForm() {
  const searchParams = useSearchParams();
  const callbackUrl = searchParams.get("callbackUrl")?.trim() || "/relays";
  const oauthError = signInErrorMessage(searchParams.get("error"));

  const [nip07Ready, setNip07Ready] = useState(false);
  const [busy, setBusy] = useState<"nostr" | null>(null);
      ...
      <div className="w-full max-w-[380px]">
        ...
        <div className="bg-card border border-border rounded-xl p-8">
          ...
          <p className="text-[13px] text-muted-foreground mb-6">
            Team access with GitHub, or your Nostr key via a NIP-07 extension.
          </p>

          {error ? (
            <p
              className="mb-4 rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-[12px] text-destructive"
              role="alert"
            >
              {error}
            </p>
          ...
          <form action={signInWithGitHub}>
            ...
            <GitHubSubmitButton disabled={busy === "nostr"} />
          </form>
...
function GitHubSubmitButton({ disabled }: { disabled: boolean }) {
  const { pending } = useFormStatus();
  const waiting = pending || disabled;
  return (
    <button
      type="submit"
      disabled={waiting}
